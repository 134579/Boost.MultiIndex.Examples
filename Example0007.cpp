...
typedef struct indices : public boost::multi_index::indexed_by<
  ...
  enum INDEX { IDX_UNIQUE_CHARID, IDX_NON_UNIQUE_LEVEL, IDX_END };
} INDICES;
...
int main() {
  ...
  std::for_each(CharacterSet.begin(), CharacterSet.end(),
                [](const CHARACTER &Char) {
                  std::cout << "CharID : " << Char.m_nID
                            << ", Level : " << Char.m_nLevel << std::endl;
                });
  std::cout << std::endl;
  ...
  Container::nth_index<indices::IDX_UNIQUE_CHARID>::type &Index1 =
      CharacterSet.get<indices::IDX_UNIQUE_CHARID>();
  Container::nth_index_iterator<indices::IDX_UNIQUE_CHARID>::type iter =
      Index1.find(3);

  if (Index1.end() != iter) {
    std::cout << "CharID : " << (*iter).m_nID << "CharID:33, Level:33, Exp:333"
              << std::endl;
  ...
  Container::nth_index<indices::IDX_UNIQUE_CHARID>::type &Index2 =
      CharacterSet.get<indices::IDX_UNIQUE_CHARID>();
  iter = Index2.find(33);
  ...
  Container::nth_index<indices::IDX_UNIQUE_CHARID>::type &Index4 =
      CharacterSet.get<indices::IDX_UNIQUE_CHARID>();
  iter = Index4.find(44);
