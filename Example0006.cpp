...
class CHARACTER {
public:
  CHARACTER(const int nLevel, const char *pszName)
      : m_nLevel(nLevel), m_Name(pszName) {}
...
typedef boost::multi_index::const_mem_fun<CHARACTER, const std::string,
                                          &CHARACTER::Name>
    Index_Ordered_Name;

typedef struct indices
    : public boost::multi_index::indexed_by<
          Index_Sequenced, boost::multi_index::ordered_non_unique<Index_Char>,
...
int main() {
  ...
  std::for_each(CharacterSet.begin(), CharacterSet.end(),
                [](const CHARACTER &Char) {
                  std::cout << "Level : " << Char.Level()
                            << ", Name : " << Char.Name() << std::endl;
                });
  std::cout << std::endl << std::endl;

  std::cout << "XXXX" << std::endl;
  Container::nth_index<indices::IDX_NON_UNIQUE_CHAR>::type &Index1 =
      CharacterSet.get<indices::IDX_NON_UNIQUE_CHAR>();
  std::for_each(Index1.begin(), Index1.end(), [](const CHARACTER &Char) {
    std::cout << "Level : " << Char.Level() << ", Name : " << Char.Name()
              << std::endl;
  });
  std::cout << std::endl;
