...
typedef boost::multi_index::const_mem_fun<CHARACTER, const int,
                                          &CHARACTER::Level>
    IDX_LEVEL;
typedef boost::multi_index::member<CHARACTER, int, &CHARACTER::m_nExp> IDX_EXP;
typedef boost::multi_index::composite_key<CHARACTER, IDX_LEVEL, IDX_EXP>
    IDX_LEVEL_EXP;
typedef boost::multi_index::composite_key<CHARACTER, IDX_EXP, IDX_LEVEL>
    IDX_EXP_LEVEL;
typedef boost::multi_index::member<CHARACTER, int, &CHARACTER::m_nID>
    IDX_CHARID;
...
int main() {
  ...
  Container::nth_index<indices::IDX_NON_UNIQUE_LEVEL_EXP>::type &Index1 =
      CharacterSet.get<indices::IDX_NON_UNIQUE_LEVEL_EXP>();
  std::for_each(Index1.begin(), Index1.end(), [](const CHARACTER &Char) {
    std::cout << "Level : " << Char.Level() << ", Exp : " << Char.Exp()
              << std::endl;
  });
  ...
  Container::nth_index<indices::IDX_NON_UNIQUE_EXP_LEVEL>::type &Index2 =
      CharacterSet.get<indices::IDX_NON_UNIQUE_EXP_LEVEL>();
  ...
  Container::iterator kb =
      CharacterSet.get<indices::IDX_NON_UNIQUE_LEVEL_EXP>().lower_bound(GeqKey);
  Container::iterator ke =
      CharacterSet.get<indices::IDX_NON_UNIQUE_LEVEL_EXP>().lower_bound(LtKey);
  while (kb != ke) {
    std::cout << "Level : " << (*kb).Level() << ", Exp : " << (*kb).Exp()
              << std::endl;
    ++kb;
  }
  std::cout << std::endl << std::endl;
  ...
  Container::nth_index<indices::IDX_UNIQUE_CHARID>::type &Index3 =
      CharacterSet.get<indices::IDX_UNIQUE_CHARID>();
  Container::nth_index_iterator<indices::IDX_UNIQUE_CHARID>::type iter =
      Index3.find(3);
  ...
  std::pair<Container::nth_index_iterator<indices::IDX_NON_UNIQUE_LEVEL>::type,
            Container::nth_index_iterator<indices::IDX_NON_UNIQUE_LEVEL>::type>
      pair;

  pair = Index4.equal_range(3);
  for (auto iter = pair.first; iter != pair.second; ++iter) {
    std::cout << "CharID : " << (*iter).m_nID
              << ", Level : " << (*iter).m_nLevel << std::endl;
  }
