...
int main() {
  ...
  typedef boost::multi_index::const_mem_fun<std::string, std::size_t,
                                            &std::string::length>
      Index_Ordered_Non_Unique_Str_Length;
  ...
  std::for_each(ItemNameContainer.begin(), ItemNameContainer.end(),
  ...
  Conainer::nth_index<indices::IDX_NON_UNIQUE>::type &Index1 =
      ItemNameContainer.get<indices::IDX_NON_UNIQUE>();
  ...
  Conainer::nth_index<indices::IDX_NON_UNIQUE_STR_LENGTH>::type &Index2 =
      ItemNameContainer.get<indices::IDX_NON_UNIQUE_STR_LENGTH>();
