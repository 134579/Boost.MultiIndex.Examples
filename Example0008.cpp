...
int main() {
  ...
  std::for_each(
      CharacterSet.begin(), CharacterSet.end(), [](const CHARACTER &Char) {
        std::cout << "CharID : " << Char.m_nID << ", Level : " << Char.m_nLevel
                  << ", Exp : " << Char.m_nExp << std::endl;
      });
  std::cout << std::endl;
  ...
  if (Index1.end() != iter) {
    ...
    Index1.modify(iter, [](CHARACTER &Char) {
      Char.m_nID = 33;
      Char.m_nLevel = 333;
    });
  }
  ...
  if (Index1.end() != iter) {
    std::cout << "CharID : " << (*iter).m_nID << "?? 6?, ??? 666?? ??"
              << std::endl;
    ...
    Index1.modify(
        ...
        [nCharID, nLevel](CHARACTER &Char) {
          Char.m_nID = nCharID;
          Char.m_nLevel = nLevel;
        });
  }
  std::cout << std::endl;
